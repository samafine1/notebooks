# Time Step Study: Variational (Adjoint) Objective
...
# Variational system setup
∂ϵH₀ = [PAULIS.X, PAULIS.Y, PAULIS.Z]
varsys = VariationalQuantumSystem(H_drive, ∂ϵH₀)

# Time step study parameters
T_values = [20, 30, 40, 50, 60, 70, 80, 90, 100, 110]
Δt_values = [TOTAL_DURATION / T for T in T_values]
n_seeds = 6
seed_values = [42+40*i for i in 1:n_seeds]

println("Time Step Study: Variational (Adjoint) Objective")
println("="^70)
...
for (i, (T, Δt)) in enumerate(zip(T_values, Δt_values))
    ...
    for (seed_idx, seed) in enumerate(seed_values)
        ...
        elapsed_time = @elapsed begin
            solve!(prob, max_iter=750, print_level=0, options=IpoptOptions(eval_hessian=false))
            solve!(prob, max_iter=30, print_level=0)
        end
...
println("\n" * "="^70)
println("VARIATIONAL TIME STEP STUDY COMPLETE")
println("="^70)
println("\nSummary Statistics:")
...
ax1 = Axis(fig[1, 1],
    title="Fidelity vs Time Steps (Variational/Adjoint)",
    xlabel="Number of Time Steps (T)",
    ylabel="Fidelity"
)
lines!(ax1, T_values, mean_fidelities, color=:purple, linewidth=3, label="Mean")
band!(ax1, T_values, 
...
lines!(ax2, T_values, mean_times, color=:orange, linewidth=3, label="Mean")
for seed_idx in 1:n_seeds
...
lines!(ax3, Δt_values, mean_fidelities, color=:teal, linewidth=3, label="Mean")
band!(ax3, Δt_values,
      mean_fidelities .- std_fidelities,
      mean_fidelities .+ std_fidelities,
      color=(:teal, 0.3))
...
summary_text = """
Variational (Adjoint) Time Step Study
...
Label(fig[0, :], "Variational (Adjoint): Time Step Study (Fixed Duration = $(TOTAL_DURATION))",
      fontsize=20, font="bold")

display(fig)

# Save results
output_dir = joinpath(@__DIR__, "timestep_study_results", "variational")
mkpath(output_dir)
...
open(summary_file, "w") do io
    println(io, "Variational (Adjoint) Time Step Study")
    println(io, "="^70)
