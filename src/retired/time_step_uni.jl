# Time Step Study: Universal Objective
...
println("Time Step Study: Universal Objective")
println("="^70)
...
for (i, (T, Δt)) in enumerate(zip(T_values, Δt_values))
    ...
    for (seed_idx, seed) in enumerate(seed_values)
        ...
        elapsed_time = @elapsed begin
            ...
            solve!(prob, max_iter=25, print_level=0)
        end
...
println("\n" * "="^70)
println("UNIVERSAL TIME STEP STUDY COMPLETE")
println("="^70)
println("\nSummary Statistics:")
...
ax1 = Axis(fig[1, 1],
    title="Fidelity vs Time Steps (Universal)",
    xlabel="Number of Time Steps (T)",
    ylabel="Fidelity"
)
lines!(ax1, T_values, mean_fidelities, color=:crimson, linewidth=3, label="Mean")
band!(ax1, T_values, 
      mean_fidelities .- std_fidelities,
      mean_fidelities .+ std_fidelities,
      color=(:crimson, 0.3), label="±1 Std")
...
lines!(ax2, T_values, mean_times, color=:darkgreen, linewidth=3, label="Mean")
for seed_idx in 1:n_seeds
...
lines!(ax3, Δt_values, mean_fidelities, color=:navy, linewidth=3, label="Mean")
band!(ax3, Δt_values,
      mean_fidelities .- std_fidelities,
      mean_fidelities .+ std_fidelities,
      color=(:navy, 0.3))
...
Label(fig[0, :], "Universal: Time Step Study (Fixed Duration = $(TOTAL_DURATION))",
      fontsize=20, font="bold")

display(fig)

# Save results
output_dir = joinpath(@__DIR__, "timestep_study_results", "universal")
mkpath(output_dir)
...
open(summary_file, "w") do io
    println(io, "Universal Time Step Study")
    println(io, "="^70)
