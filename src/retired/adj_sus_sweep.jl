...
function optimize_trajectory(a_bound::Float64, dda_bound::Float64, 
    ...
    # Use random Δt within specified range
    var_count = length(varsys.G_vars)
    ...
    push!(
        ...
        FinalUnitaryFidelityConstraint(U_goal, :Ũ⃗, F, var_prob.trajectory)
    );

    # Solve with two-stage optimization
    solve!(var_prob, max_iter=2500, print_level=0, options=IpoptOptions(eval_hessian=false))
...
folder_name = "adj_sus_dda_sweep_$(timestamp)"
...
@save joinpath(save_path, "sweep1_dda.jld2") trajectories_dda susceptibilities_dda objective_values_dda fidelities_dda susceptibilities_dda_mean susceptibilities_dda_std objective_values_dda_mean objective_values_dda_std fidelities_dda_mean fidelities_dda_std dda_bounds a_bound_fixed seed_values
@save joinpath(save_path, "sweep2_a.jld2") trajectories_a susceptibilities_a objective_values_a fidelities_a susceptibilities_a_mean susceptibilities_a_std objective_values_a_mean objective_values_a_std fidelities_a_mean fidelities_a_std a_bounds dda_bound_fixed seed_values
